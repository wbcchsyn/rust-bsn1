...
#[derive(bsn1_serde::Serialize)]
#[bsn1_serde(id = UTF8String)]
struct C {
    #[bsn1_serde(skip, skip_serializing)]
    _x: i32,
    y: String,
}
...
#[derive(bsn1_serde::Serialize)]
#[bsn1_serde(id = UTF8String, tag_class = PRIVATE)]
struct E {
    x: i32,
    #[bsn1_serde(skip)]
    _y: String,
...
struct F(#[bsn1_serde(skip)] i32);

#[derive(bsn1_serde::Serialize)]
#[bsn1_serde(id = UTF8String, tag_pc = PRIMITIVE)]
struct G(#[bsn1_serde(skip, skip_serializing)] i32, String);

#[derive(bsn1_serde::Serialize)]
#[bsn1_serde(tag_class = PRIVATE, tag_pc = PRIMITIVE)]
struct H(i32, #[bsn1_serde(skip, skip_serializing)] String);

#[derive(bsn1_serde::Serialize)]
#[bsn1_serde(id = UTF8String, tag_class = PRIVATE, tag_pc = PRIMITIVE)]
struct I(i32, #[bsn1_serde(skip)] String, bool);
...
enum X {
    ...
    #[bsn1_serde(id = UTF8String, tag_num = 1)]
    K {
        #[bsn1_serde(skip)]
        _x: i32,
        y: String,
    ...
    #[bsn1_serde(id = UTF8String, tag_class = PRIVATE, tag_num = 0x1f)]
    M {
        x: i32,
        #[bsn1_serde(skip, skip_serializing)]
        _y: String,
    ...
    N(#[bsn1_serde(skip, skip_serializing)] i32),
    #[bsn1_serde(id = UTF8String, tag_pc = PRIMITIVE, tag_num = 0x80)]
    O(#[bsn1_serde(skip)] i32, String),
    #[bsn1_serde(tag_class = PRIVATE, tag_pc = PRIMITIVE, tag_num = 0x3fff)]
    P(i32, #[bsn1_serde(skip)] String),
    #[bsn1_serde(
    id = UTF8String,
    tag_class = PRIVATE,
    tag_pc = PRIMITIVE,
    tag_num = 0x4000)]
    Q(i32, #[bsn1_serde(skip, skip_serializing)] String, bool),
}
...
fn test_c() {
    ...
    assert_eq!(der.id().len(), val.id_len().unwrap().unwrap());
    assert_eq!(der.id().class(), IdRef::utf8_string().class());
    assert_eq!(der.id().pc(), IdRef::utf8_string().pc());
    assert_eq!(
        der.id().number().unwrap(),
        IdRef::utf8_string().number().unwrap()
    );
